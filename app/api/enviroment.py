...
async def get_list_enviroment(db: Session = Depends(get_db), payload: dict = Depends(verify_token)):
...
async def get_environment_by_type( env: EnvironmentType, db: Session = Depends(get_db), payload: dict = Depends(verify_token)):
...
async def get_env_by_id(env_id:int , db: Session = Depends(get_db), payload: dict = Depends(verify_token)):
...
async def create_new_environment(environment: EnvironmentCreate, db: Session = Depends(get_db), payload: dict = Depends(verify_token)):
...
async def delete_environments(env_id:int, db: Session = Depends(get_db), payload: dict = Depends(verify_token) ):
